...
모든 서비스 간 통신은 **OpenFeign + Eureka** 기반으로 서비스명을 사용합니다. <br>
...
## 하은영 | 기본 제공 서버 분석 & Swagger 명세

기본 제공된 카드사/은행 서버를 분석하고 포트 불일치 이슈를 발견 및 수정, Swagger 명세 작성을 담당.
...
| `card-authorization-service` | 포트 `9090` → `8083` / `BankClient` URL `8080` → `8084` |

### Swagger 명세 작성
- `OpenApiConfig` 클래스 추가, `HealthController` (`GET /api/health`) 추가
- `@Operation`, `@ApiResponses`, `@ExampleObject`로 각 엔드포인트 명세 작성
- VAN Gateway 결제 승인 조회 API Swagger 문서 및 예시를 ISO 8583 DE11(STAN) 기반으로 수정

| 서비스 | Swagger UI |
|--------|-----------|
| POS Service | `http://localhost:8081/swagger-ui/index.html` |
| VAN Service | `http://localhost:8082/swagger-ui/index.html` |
| Card Authorization Service | `http://localhost:8083/swagger-ui/index.html` |
| Bank Service | `http://localhost:8084/swagger-ui/index.html` |

---

## 유승준 | POS & VAN 로직 개발 & Eureka 서버 보조
